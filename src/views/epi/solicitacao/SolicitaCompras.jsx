...
const SolicitaCompras = _ => {
    //Usuário logado
    const user = Session.getSession();

    // Selects com objetos carregados pelo Api.
    const [categorias, setCategorias] = useState([]);
    const [produtos, setProdutos] = useState([]);
    const [motivos, setMotivos] = useState([]);
    const [pedidosInseridos, setPedidosInseridos] = useState([]);
    ...
    const [pedidoInseridoId, setPedidoInseridoId] = useState(0);
    ...
    const handlePedidoInserido = async (e) => {
        setPedidoInseridoId(e.target.value);
    }

    const handleAtualizacaoQuantidade = async (e) => {
    ...
    const adicionaPedido = async (e) => {
        Swal.fire({
            didOpen: async () => {
                Swal.showLoading()
                const res = await Api.queryGet(`/pedidos/${pedidoInseridoId}`);
                if(res.result){
                    res.pedido.produtos.map((item) => {

                        let found = listaItens.findIndex(element => element.nome === item.nome);
                        if(found > -1) {
                            var valueUpdate = {
                                id: item.id,
                                nome: item.nome,
                                quantidade: (item.quantidade + listaItens[found].quantidade)
                            }

                            listaItens[found] = valueUpdate;
                            setListaItens(listaItens)
        ...
        setTabela(listaItens.map((item, i) => {
            return (
                <tr key={`listaItens${item.id}`}>
                    ...
                    <td className='text-center'><button type="button" className='btn btn-inverse-primary' onClick={e => handleAtualizacaoQuantidade(i)}>Editar Quantidade</button></td>
    ...
    useEffect(() => {
        ...
        const loadPedidosInseridos = async (e) => {
            const res = await Api.queryGet(`/pedidos/status/${8}`);
            if(res.result){
                setPedidosInseridos(res.lista);
            }
        }
        
        loadCategorias();
        loadMotivos();
        loadPedidosInseridos();
    }, []);
        ...
        <div className="col-lg-12 stretch-card">
            <div className='card'>
                <div className='card-body'>
                    <h4 className='card-title'>Inserir Pedido de Compras</h4>
                    <form noValidate className="forms-sample" id="salvaCategoria">
                        ...
                        <div className='row'>
                            <div className='col-lg-4'>
                                <div className="form-group">
                                    <label htmlFor="categoria">Adicionar a partir de um pedido</label>
                                    <select className="form-control" defaultValue="0" id="categoria" onChange={e => handlePedidoInserido(e)}>
                                        <option value="0">Selecione</option>
                                        {  
                                            pedidosInseridos.length > 0 &&
                                            pedidosInseridos.map((item) => {
                                                return (
                                                    <option key={"pedidosInseridos"+item.id} value={item.id}>{item.usuario+" - "+item.nome}</option>
                                                )
                                            })
                                        }
                                    </select>
                            ...
                            <div className='col-lg-2'>
                                <button type="button" className="btn btn-inverse-primary mr-2" onClick={e => adicionaPedido()}>Adicionar o Pedido</button>
                            </div>
                        </div>
                        <div className='row'>
                            <div className='col-lg-4'>
                        ...
                        <div className="row">
                            <div className='col-lg-6'>
                                <Link to={`/listaCompras/`} className="btn btn-inverse-secondary">Voltar</Link>
                            </div>
...
export default SolicitaCompras