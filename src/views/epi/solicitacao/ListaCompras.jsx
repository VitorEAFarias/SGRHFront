        ...
        const loadSolicitacoes = async () => {
            ...
            if(res.result) {
                setSolicitacoes(await res.lista.map((solicitacao) => {
                    return(
                        {
                            info: <Link to={`/detalhesCompras/${solicitacao.id}`} className="btn btn-inverse-primary btn-rounded btn-fw"><i className='mdi mdi-information-outline'></i></Link>,
        ...
        <div className="col-lg-12 stretch-card">
            <div className='card'>
                <div className='card-body'>
                    <h4 className='card-title'>Lista de Compras</h4>
                    <div className="row">
                        <div className='col-lg-12'>
                            <Link to={`/solicitaCompras/`} className="btn btn-inverse-primary">Inserir Solicitação de Compras</Link>
                        </div>
                    </div>
                    <DataTable 
                        columns={columns} 
